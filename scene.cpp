...
void scene::load_texture(string Filename, int index)
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, width, height,	0, GL_BGRA, GL_UNSIGNED_BYTE, (void*)FreeImage_GetBits(p32BitsImage));
	glGenerateMipmap(GL_TEXTURE_2D);
	//gluBuild2DMipmaps(GL_TEXTURE_2D, 4, width, height, GL_BGRA, GL_UNSIGNED_BYTE, (void*)FreeImage_GetBits(p32BitsImage));
...
void scene::load_cube(string Filename[6], int index)
{
	...
	for (int i = 0; i<6; i++) 
	{
		...
		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X+i, 0, GL_RGB, width, height, 0, GL_BGRA, GL_UNSIGNED_BYTE, (void*)FreeImage_GetBits(p32BitsImage));
		glGenerateMipmap(GL_TEXTURE_CUBE_MAP);
		//gluBuild2DMipmaps(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 4, width, height, GL_BGR, GL_UNSIGNED_BYTE, (void*)FreeImage_GetBits(pImage));
